...
export let sidebarDoc = function (doc, options) {
  if (options == null) {
    options = {};
  }
...
export let sidebarType = type => `<a href="${type.fullPath()}" class="_list-item _list-dir" data-slug="${type.slug}" tabindex="-1">${arrow}<span class="_list-count">${type.count}</span><span class="_list-text">${$.escape(type.name)}</span></a>`;

export let sidebarEntry = entry => `<a href="${entry.fullPath()}" class="_list-item _list-hover" tabindex="-1">${$.escape(entry.name)}</a>`;

export let sidebarResult = function (entry) {
...
export let sidebarNoResults = function () {
...
export let sidebarPageLink = count => `<span role="link" class="_list-item _list-pagelink">Show more\u2026 (${count})</span>`;

export let sidebarLabel = function (doc, options) {
  if (options == null) {
    options = {};
  }
...
export let sidebarVersionedDoc = function (doc, versions, options) {
  if (options == null) {
    options = {};
  }
...
export let sidebarDisabled = options => `<h6 class="_list-title">${arrow}Disabled (${options.count}) <a href="/settings" class="_list-title-link" tabindex="-1">Customize</a></h6>`;

export let sidebarDisabledList = html => `<div class="_disabled-list">${html}</div>`;

export let sidebarDisabledVersionedDoc = (doc, versions) => `<a class="_list-item _list-dir _icon-${doc.icon} _list-disabled" data-slug="${doc.slug_without_version}" tabindex="-1">${arrow}${doc.name}</a><div class="_list _list-sub">${versions}</div>`;

export let docPickerHeader = "<div class=\"_list-picker-head\"><span>Documentation</span> <span>Enable</span></div>";

export let docPickerNote = `\
